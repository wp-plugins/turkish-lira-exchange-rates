...
#currency div.c_row div.c_rate{min-width:50px;float:right;text-align:right;padding:0px 4px;}
...
#currency div.c_credit a{color:#369;clear:both;text-align:center;font-weight:bold;}
		...
		while (!feof($rates)) {
			...
			if ($findAUD=strpos($row,$AUD)===false){
				...
				$AUDbuy=number_format(trim(substr($row,$findAUD+46,6)),4);
				$AUDsell=number_format(trim(substr($row,$findAUD+61,6)),4);
			}
			if ($findCAD=strpos($row,$CAD)===false){
			}else{
				$findCAD=strpos($row,$CAD);
				$CADbuy=number_format(trim(substr($row,$findCAD+46,6)),4);
				$CADsell=number_format(trim(substr($row,$findCAD+61,6)),4);
			}
			if ($findCHF=strpos($row,$CHF)===false){
			}else{
				$findCHF=strpos($row,$CHF);
				$CHFbuy=number_format(trim(substr($row,$findCHF+46,6)),4);
				$CHFsell=number_format(trim(substr($row,$findCHF+61,6)),4);
			}
			if ($findDKK=strpos($row,$DKK)===false){
			}else{
				$findDKK=strpos($row,$DKK);
				$DKKbuy=number_format(trim(substr($row,$findDKK+46,6)),4);
				$DKKsell=number_format(trim(substr($row,$findDKK+61,6)),4);
			}
			if ($findEUR=strpos($row,$EUR)===false){
			}else{
				$findEUR=strpos($row,$EUR);
				$EURbuy=number_format(trim(substr($row,$findEUR+46,6)),4);
				$EURsell=number_format(trim(substr($row,$findEUR+61,6)),4);
			}
			if ($findGBP=strpos($row,$GBP)===false){
			}else{
				$findGBP=strpos($row,$GBP);
				$GBPbuy=number_format(trim(substr($row,$findGBP+46,6)),4);
				$GBPsell=number_format(trim(substr($row,$findGBP+61,6)),4);
			}
			if ($findJPY=strpos($row,$JPY)===false){
			}else{
				$findJPY=strpos($row,$JPY);
				$JPYbuy=number_format(trim(substr($row,$findJPY+46,6)),4);
				$JPYsell=number_format(trim(substr($row,$findJPY+61,6)),4);
			}
			if ($findKWD=strpos($row,$KWD)===false){
			}else{
				$findKWD=strpos($row,$KWD);
				$KWDbuy=number_format(trim(substr($row,$findKWD+46,6)),4);
				$KWDsell=number_format(trim(substr($row,$findKWD+61,6)),4);
			}
			if ($findNOK=strpos($row,$NOK)===false){
			}else{
				$findNOK=strpos($row,$NOK);
				$NOKbuy=number_format(trim(substr($row,$findNOK+46,6)),4);
				$NOKsell=number_format(trim(substr($row,$findNOK+61,6)),4);
			}
			if ($findSAR=strpos($row,$SAR)===false){
			}else{
				$findSAR=strpos($row,$SAR);
				$SARbuy=number_format(trim(substr($row,$findSAR+46,6)),4);
				$SARsell=number_format(trim(substr($row,$findSAR+61,6)),4);
			}
			if ($findSEK=strpos($row,$SEK)===false){
			}else{
				$findSEK=strpos($row,$SEK);
				$SEKbuy=number_format(trim(substr($row,$findSEK+46,6)),4);
				$SEKsell=number_format(trim(substr($row,$findSEK+61,6)),4);
			}
			if ($findUSD=strpos($row,$USD)===false){
			}else{
				$findUSD=strpos($row,$USD);
				$USDbuy=number_format(trim(substr($row,$findUSD+46,6)),4);
				$USDsell=number_format(trim(substr($row,$findUSD+61,6)),4);
			}
